...
var Model = EmitterClass.extends({
	$ : function( conn ){
		...
		for(var key in this.methods){
			this.bindMethod( key , this.methods[key]);
		}
		//create the db
	...
	bindMethod: function( key, fn ){
		this._schema.method(key,fn);
	},
    ...
    findOneAndRemove: function(){
        return this.db.findOneAndRemove.apply(this.db,arguments);
    },
    toResult: function( callback ){
        ...
        return function( err, result ){
            ...
            if(_.isArray(result)){
                ...
                if(!_.isNull(result) && _.isFunction(result.toJSON)){
...
Model.define = function( name, schema , before, after , method, collection , toJSON ){
	before = before || {};
	after = after || {};
	method = method || {};
