...
var iZhui = Class.extends({
    ...
    start: function( path ){
		...
		this.errordir = Path.join(path,"../.error");
	...
	rollback: function(){
		this.files.forEach(function(item,index){
			var rable = fs.createReadStream(item.from);
			var wable = fs.createWriteStream(item.error);
			rable.pipe(wable);
			debug("转移文件到:->错误文件缓存",item.from,"->",item.error);
		});
		this.files.forEach(function(item,index){
			var rable = fs.createReadStream(item.to);
			var wable = fs.createWriteStream(item.from);
			rable.pipe(wable);
			debug("转移文件到:->正确文件回滚",item.to,"->",item.from);
		});
    ...
    loadScript: function( paths, base, file ){
		...
		var errdir = Path.join(this.errordir,paths.join("/"));
		...
		var efile = Path.join(errdir,base+".js");
        var writable = fs.createWriteStream( to );
        readable.pipe(writable);
        debug("copy-file",file,"->",to);
		this.files.push({ from: file, to: to , error: efile});
        // console.log(dir);
    },
...
process.on("uncaughtException",function(err){
	...
	debug("restart",Date.now());
	exports.rollback();
});

module.exports = exports = new iZhui;
